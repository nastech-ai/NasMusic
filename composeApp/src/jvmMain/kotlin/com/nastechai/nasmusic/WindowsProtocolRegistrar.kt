...
/**
 * Registers the "nasmusic://" custom URI protocol handler in Windows Registry
 * under HKEY_CURRENT_USER (no admin rights required).
 *
 * Registry structure:
 * ```
 * HKCU\Software\Classes\nasmusic
 *     (Default) = "URL:NasMusic Protocol"
 *     URL Protocol = ""
 *     \DefaultIcon
 *         (Default) = "\"<exe_path>\",0"
 *     \shell\open\command
...
object WindowsProtocolRegistrar {
    ...
    private const val SCHEME = "nasmusic"
    ...
    fun register() {
        ...
        try {
            ...
            Logger.d(TAG, "Registering nasmusic:// protocol handler -> $exePath")

            // Main key with protocol description
            regAdd(REG_KEY, null, "URL:NasMusic Protocol")
    ...
    private fun isAlreadyRegistered(currentExePath: String): Boolean {
        return try {
            ...
            // Registry stores path with quotes: "C:\path\to\NasMusic.exe" "%1"
    ...
    private fun resolveExePath(): String? {
        ...
        //   <app>/NasMusic.exe
        // So we go: java.home → parent (runtime) → parent (app) → NasMusic.exe
        ...
        if (appDir != null) {
            val exeFile = java.io.File(appDir, "NasMusic.exe")
            if (exeFile.exists()) {
