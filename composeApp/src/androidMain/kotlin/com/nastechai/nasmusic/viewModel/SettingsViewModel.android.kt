...
import com.nastechai.common.DOWNLOAD_EXOPLAYER_FOLDER
import com.nastechai.common.EXOPLAYER_DB_NAME
...
import com.nastechai.nasmusic.extension.getSizeOfFile
import com.nastechai.nasmusic.extension.zipInputStream
...
actual suspend fun calculateDataFraction(cacheRepository: CacheRepository): SettingsStorageSectionFraction? {
    ...
    return withContext(Dispatchers.Default) {
        ...
        if (mStorageStatsManager != null) {
            ...
            val nasMusicSize = getSizeOfFile(application.filesDir).bytesToMB()
            val thumbSize = (application.imageLoader.diskCache?.size ?: 0L).bytesToMB()
            val otherApp = nasMusicSize.let { usedSpace.minus(it) - thumbSize }
            val databaseSize =
                nasMusicSize - playerCache.bytesToMB() - downloadCache.bytesToMB() - canvasCache.bytesToMB()
            if (totalByte ==
                freeSpace + otherApp + nasMusicSize + thumbSize
            ) {
